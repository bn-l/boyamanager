...
* A mini 2 exposes **24 attributes** — see **§9.5**, which is the definitive list for
  ...
  (68 — Mono/Stereo/Safety Channel) and mute (48).
...
**This is the list that matters for this device — 24 attributes.** Every row here was
returned by the receiver itself. Anything *not* in this table is in §9.6 and does not
exist on a mini 2 — including `agc` (46), which BOYA's per-model metadata claims but
the device never answers for (see below).
...
`recording_mode` (Mono / Stereo / Safety Channel) and `mute`.
...
**`agc` (46) — resolved: absent, not merely gated on a transmitter.** An earlier
revision assumed `agc` returns `status 1` only while no transmitter is connected.
It does not come back at all: with TX2 online and healthy (battery 3, signal 4),
`get_all` omits it *and* a direct `0x1F` read answers `status 1`. Treat it as not
implemented on a mini 2 RX, whatever BOYA's per-model metadata says. It has been
dropped from the table above and from `BoyaManager`'s attribute enum entirely;
a hardware test still reads id 46 directly and asserts `status 1`, so a firmware
update that starts answering shows up as a failure rather than silently.
...
That is all 24 attributes in §9.5. `get_all` only returns what is currently
meaningful, so a docked transmitter's own settings can still drop out.
...
* ~~Two settable attributes are still unwritten~~ — **resolved**.
  `tx_auto_poweroff` (45) has been written and read back; `agc` (46) turned out
  not to exist on this model at all (§9.5). Every `RW` row in §9.5 has now been
